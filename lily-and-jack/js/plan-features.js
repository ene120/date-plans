  ...
  window.__pf = {
    ...
    getFoodRecsHtml(stop) {
      ...
      if (f[P.partner1.toLowerCase()]) {
        h += `<div class="pf-food-partner"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#e84393" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px"><path d="M12 2c0 4-4 6-4 10a4 4 0 008 0c0-4-4-6-4-10z"/></svg>${P.partner1}'s Picks</div>`;
      ...
      if (f[P.partner2.toLowerCase()]) {
        h += `<div class="pf-food-partner"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="#e84393" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>${P.partner2}'s Picks</div>`;
      ...
      if (f.shared) {
        h += `<div class="pf-food-partner"><svg viewBox="0 0 24 24" width="14" height="14" fill="#e84393" stroke="#e84393" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>Share Together</div>`;
    ...
    getActionBtnsHtml(stop) {
      ...
      const ic = (d,s='18') => `<svg viewBox="0 0 24 24" width="${s}" height="${s}" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:4px">${d}</svg>`;
      if (stop.menuUrl) h += `<a class="pf-action" href="${stop.menuUrl}" target="_blank">${ic('<path d="M4 19.5A2.5 2.5 0 016.5 17H20"/><path d="M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z"/>')}Menu</a>`;
      if (stop.reservationUrl) h += `<a class="pf-action" href="${stop.reservationUrl}" target="_blank">${ic('<rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/>')}Reserve</a>`;
      if (stop.phone) h += `<a class="pf-action" href="tel:${stop.phone}">${ic('<path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72 12.84 12.84 0 00.7 2.81 2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45 12.84 12.84 0 002.81.7A2 2 0 0122 16.92z"/>')}Call</a>`;
      if (stop.transit) {
        if (stop.transit.uber) h += `<a class="pf-action" href="${stop.transit.uber}" target="_blank"><img src="assets/uber-logo.png" alt="Uber" style="width:16px;height:16px;object-fit:contain;vertical-align:middle;margin-right:4px">Uber</a>`;
        if (stop.transit.lyft) h += `<a class="pf-action" href="${stop.transit.lyft}" target="_blank"><img src="assets/lyft-logo.png" alt="Lyft" style="width:16px;height:16px;object-fit:contain;vertical-align:middle;margin-right:4px">Lyft</a>`;
      }
      if (stop.happyHour) h += `<span class="pf-action">${ic('<path d="M18 8h1a4 4 0 010 8h-1"/><path d="M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/>')}Happy Hour: ${stop.happyHour.times}</span>`;
      return h + '</div>';
    },
    ...
    getParkingHtml(stop) {
      ...
      let h = `<div class="pf-parking">
        <button class="pf-parking-btn" onclick="this.classList.toggle('open');document.getElementById('${id}').classList.toggle('open')">
          <span><span class="pf-park-icon"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="2" width="20" height="20" rx="4"/><path d="M9 17V7h4a3 3 0 010 6H9"/></svg></span> Parking Options · ${rec.cost}</span>
      ...
      opts.forEach(o => {
        ...
        if (o.tip) h += `<div class="pf-park-tip"><svg viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="#d4a810" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:middle;margin-right:3px"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 00-4 12.7V17h8v-2.3A7 7 0 0012 2z"/></svg>${o.tip}</div>`;
